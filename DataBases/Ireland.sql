...
AS
	BEGIN TRY
		...
		BEGIN TRANSACTION TS;
			...
			BEGIN
				INSERT INTO @temporal2(id_tmp) SELECT * FROM openquery(SQLSERVER,' SELECT Identification FROM employee.employeesinfo WHERE Position_id = 1 AND Country_id = 3 AND Shop_id = 1;') --corregir empleados depende del pais y tienda donde trabajen
				SET @tmp_id2 = (SELECT TOP(1) id_tmp FROM @temporal2 ORDER BY NEWID())
			END
			ELSE IF @in_countryID = '3' AND @in_shopID = '2'
			BEGIN
				INSERT INTO @temporal2(id_tmp) SELECT * FROM openquery(SQLSERVER,' SELECT Identification FROM employee.employeesinfo WHERE Position_id = 1 AND Country_id = 3 AND Shop_id = 2;') 
				SET @tmp_id2 = (SELECT TOP(1) id_tmp FROM @temporal2 ORDER BY NEWID())
			END
			ELSE IF @in_countryID = '3' AND @in_shopID = '3'
			BEGIN
				INSERT INTO @temporal2(id_tmp) SELECT * FROM openquery(SQLSERVER,' SELECT Identification FROM employee.employeesinfo WHERE Position_id = 1 AND Country_id = 3 AND Shop_id = 3;') 
				SET @tmp_id2 = (SELECT TOP(1) id_tmp FROM @temporal2 ORDER BY NEWID())
			END
