...
CREATE PROCEDURE FinishPurchase --USA 1, SCOTLAND 2, IRELAND 3
...
AS
	BEGIN TRY
		...
		(id_tmp VARCHAR(50))
		DECLARE @total MONEY, @tmp_id2 VARCHAR(50) = 'EMPTY'
		BEGIN TRANSACTION TS;
			IF @in_countryID = '1' AND @in_shopID = '1'
			BEGIN
				INSERT INTO @temporal2(id_tmp) SELECT * FROM openquery(SQLSERVER,' SELECT Identification FROM employee.employeesinfo WHERE Position_id = 1 AND Country_id = 1 AND Shop_id = 1;') --corregir empleados depende del pais y tienda donde trabajen
				SET @tmp_id2 = (SELECT TOP(1) id_tmp FROM @temporal2 ORDER BY NEWID())
			END
			ELSE IF @in_countryID = '1' AND @in_shopID = '2'
			BEGIN
				INSERT INTO @temporal2(id_tmp) SELECT * FROM openquery(SQLSERVER,' SELECT Identification FROM employee.employeesinfo WHERE Position_id = 1 AND Country_id = 1 AND Shop_id = 2;') 
				SET @tmp_id2 = (SELECT TOP(1) id_tmp FROM @temporal2 ORDER BY NEWID())
			END
			ELSE IF @in_countryID = '1' AND @in_shopID = '3'
			BEGIN
				INSERT INTO @temporal2(id_tmp) SELECT * FROM openquery(SQLSERVER,' SELECT Identification FROM employee.employeesinfo WHERE Position_id = 1 AND Country_id = 1 AND Shop_id = 3;') 
				SET @tmp_id2 = (SELECT TOP(1) id_tmp FROM @temporal2 ORDER BY NEWID())
			END
...
-- EXECUTE GeneratePurchase '2020'
-- EXECUTE AddKart 'Chivas Regal', '2', '1', '2020'
